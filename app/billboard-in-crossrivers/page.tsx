...
export default function BillboardInCrossRiversPage() {
  const billboards = Array.from({ length: 6 }, (_, i) => ({
    id: i + 1,
    title: `Billboard Location ${i + 1} in Cross Rivers`,
    type: ["Unipole", "Gantry", "48 Sheet"][i % 3],
    area: ["Calabar", "Ikom", "Ogoja"][i % 3],
    available: i % 2 === 0,
  }))
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-6xl mx-auto">
            <div className="flex items-center gap-4 mb-8">
              ...
              <div>
                <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold">Billboards in Cross Rivers</h1>
                <p className="text-lg text-muted-foreground mt-2">65 billboard locations available</p>
              </div>
            </div>
              ...
              {billboards.map((billboard) => (
                <Card key={billboard.id} className="overflow-hidden hover:shadow-lg transition-all">
                  <div className="relative h-48">
                    <Image
                      src={`/billboard-.jpg?height=200&width=300&query=billboard+${billboard.area}+Cross+Rivers`}
                      alt={billboard.title}
                      fill
