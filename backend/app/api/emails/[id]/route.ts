...
export async function GET(_req: Request, { params }: { params: { id: string } }) {
  const [rows] = await db.query("SELECT * FROM `emails` WHERE `ID` = ? LIMIT 1", [params.id]);
...
export async function PUT(req: Request, { params }: { params: { id: string } }) {
  ...
  const [result] = await db.query("UPDATE `emails` SET ? WHERE `ID` = ?", [data, params.id]);
...
export async function DELETE(_req: Request, { params }: { params: { id: string } }) {
  const [result] = await db.query("DELETE FROM `emails` WHERE `ID` = ?", [params.id]);
  return NextResponse.json({ success: true, result });
}
